...
const Navbar = ({userDetails, userId, cookies, role, setCookies, setUserDetails}) => {
  ...
  return (
    <div className='flex flex-row items-center w-full px-[20px] py-[30px] mb-4 bg-primary-100 sm:flex-col sm:items-start sm:p-3'>
      ...
      {(cookies.access_token && userId && role == 1) && (<div className='border text-white mx-auto px-4 py-2 sm:mt-4 rounded-md bg-slate-700'>{userDetails?userDetails.email:""} - {userDetails?userDetails.firstName?.toString().toUpperCase():""}</div>)}
      {(cookies.access_token && userId && role == 2) && (<div className='border text-white mx-auto px-4 py-2 sm:mt-4 rounded-md bg-slate-700'>{userDetails?userDetails.matricNo?.toString().toUpperCase():""} - {userDetails?userDetails.firstName?.toString().toUpperCase():""}</div>)}
    </div>
  )
}
