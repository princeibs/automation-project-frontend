...
const Navbar = ({userDetails, userId, cookies, role, setCookies, setUserDetails}) => {
  ...
  return (
    <div className='flex flex-row items-center w-full px-[20px] py-[30px] mb-4 bg-primary-100 sm:flex-col sm:items-start sm:p-3'>
      ...
      <div className='border-2 font-bold border-primary-300 flex flex-row justify-around items-center w-[700px] h-[50px] rounded-[10px] ml-[6rem] no-underline sm:m-0 sm:w-full sm:border-0 sm:flex-wrap sm:h-auto sm:items-start sm:mt-4 sm:gap-1'>  
        ...
        {(userDetails.role == 1) && <Link className='bg-primary-100 sm:bg-primary-0 sm:rounded-md sm:px-6 sm:py-2 drop-shadow-[0] sm:drop-shadow' to="/staff/students">Students</Link>}
        {(userDetails.role == 1) && <Link className='bg-primary-100 sm:bg-primary-0 sm:rounded-md sm:px-6 sm:py-2 drop-shadow-[0] sm:drop-shadow' to="/staff/topics">Topics</Link>}
        {(userDetails.role == 1) && <Link className='bg-primary-100 sm:bg-primary-0 sm:rounded-md sm:px-6 sm:py-2 drop-shadow-[0] sm:drop-shadow' to="/staff/add">Add</Link>}

        {/* Admin */}
        {(userDetails.role == 0) && <Link className='bg-primary-100 sm:bg-primary-0 sm:rounded-md sm:px-6 sm:py-2 drop-shadow-[0] sm:drop-shadow' to="/admin/students">Students</Link> }
        {(userDetails.role == 0) && <Link className='bg-primary-100 sm:bg-primary-0 sm:rounded-md sm:px-6 sm:py-2 drop-shadow-[0] sm:drop-shadow' to="/admin/supervisors">Supervisors</Link> }
