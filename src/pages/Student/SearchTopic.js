...
const Search = ({ handleSearch, loading }) => {
  ...
  return (
    <div className="w-auto sm:w-full sm:px-3">
      ...
      <p className="mx-auto mb-[2rem] w-fit text-center">
        Enter the description of the topic you want to work on
      </p>
      <form className="w-[50rem] sm:w-full m-auto" onSubmit={formSubmit}>
        <label
          for="areaOfSpecialization"
        ...
        <label for="" class="mb-2 font-semibold text-gray-900">
          Level of Expertise
        </label>
        <select
          id=""
          onChange={(e) => setLevelOfExpertise(e.target.value)}
          class="bg-gray-50 mb-6 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
          required
        >
        ...
        <p className="text-sm mb-2">
          Which of the programming languages/tools are you familar with?
        </p>
        ...
        <label for="" class="mb-1 font-semibold text-gray-900">
          Search Type
        </label>
        <div className="flex justify-between gap-5">
          <div className="flex flex-col w-[50%]">
            ...
            <p className="text-xs">
              <span className="underline text-yellow-600">*Note: </span>Use
              Narrow Search if topics must match ALL the search parameters above
            </p>
          </div>
          <div className="flex flex-col w-[50%]">
            ...
            <p className="text-xs">
              <span className="underline text-yellow-600">*Note: </span>Use
              Broad Search if you want topics to match ATLEAST ONE of the search
              parameters above
            </p>
          </div>
        </div>
        <button
          disabled={loading}
          type="submit"
          class="w-full h-[3rem] mt-[2rem] text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center disabled:bg-primary-300"
        >
          {loading ? (
            <svg
              aria-hidden="true"
...
const Recommended = ({
  ...
  return (
    <>
      {!loading ? (
        <div className="px-3">
          <div className="text-4xl mx-auto mb-[1rem] w-fit text-center">
            Recommended Topics ({topics?.length})
          </div>
          {searchType == "narrow" && (
            <div className="mx-auto mb-[2rem] text-green-900 text-sm">
              <p className="text-base font-bold">
                Topics must meet ALL of the following criteria:
              </p>
              <p>1. {String(expertise).toUpperCase()} skill level.</p>
              <p>
                2. Can be implemented using any of the tools/languages:{" "}
                {tools.toString().toUpperCase()}.
              </p>
          ...
          {searchType == "broad" && (
            <div className="mx-auto mb-[2rem] text-green-900 text-sm">
              <p className="text-base font-bold">
                Topics must meet ATLEAST ONE of the following criteria:
              </p>
              <p>1. {String(expertise).toUpperCase() || "_"} skill level.</p>
              <p>
                2. Can be implemented using any of the tools/languages:{" "}
                {tools.toString().toUpperCase() || "_"}.
              </p>
              <p>
                3. It is {specialization.toString().toUpperCase() || "_"} based.
              </p>
            </div>
          )}
          {/* Topics */}
          <div className="m-auto w-fit flex gap-3 flex-col">
            {topics.length == 0 && (
              <div className="underline my-4">
                The criteria above did not match any topic in the database
              </div>
            )}
            {topics.length > 0 && (
              <>
                {topics.map((topic) => (
                  <div className="border rounded p-4 w-[50rem] sm:w-full bg-yellow-50">
                    <div className="flex justify-between items-start mb-5 mt-2 mr-2">
                      ...
                      <button
                        disabled={details.selectedTopic}
                        onClick={() =>
                          selectTopic(topic._id, topic.createdBy._id)
                        }
                        type="button"
                        class="focus:outline-none text-white bg-green-700 disabled:bg-gray-200 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2"
                      >
                        Select Topic
                      </button>
                    </div>
                      ...
                      <div className="flex gap-4">
                        <div className="flex flex-wrap gap-2">
                          {topic?.categories
                            ...
                            .split(",")
                            .map((catg) => (
                              <div className="border border-primary-500 rounded-md h-fit px-2 py-1">
                                {catg}
                              </div>
                            ))}
                        </div>
                        {"|"}
                        <div className="border border-orange-300 rounded-md h-fit w-fit px-2 py-1">
                          {topic.expertise}
                        </div>
                        {"|"}
                        <div className="flex flex-wrap gap-2">
                          {topic.tools
                            ...
                            .split(",")
                            .map((lang) => (
                              <div className="border border-teal-500 rounded-md px-2 py-1">
                                {lang}
                              </div>
                            ))}
                        </div>
                      ...
                      <Link to={`/staff-details/${topic.createdBy._id}`}>
                        <div className="flex gap-3 items-center justify-center hover:underline">
                          <img
                            ...
                            src={topic.createdBy.image}
                          />
                          {topic.createdBy.title} {topic.createdBy.firstName}{" "}
          ...
          <div className="m-auto w-fit mt-[3rem]">
            Can't find a suitable topic?{" "}
            <button onClick={showSearch} className="underline text-primary-500">
              Search again
            </button>
          </div>
        </div>
  ...
  const handleSearch = async (
    ...
    if (searchType == "narrow") {
      if (!areaOfSpecialization) {
        toast.warn(
          "To perform a Narrow Search, you must select area of specialization"
        );
        return;
      }
      if (!levelOfExpertise) {
        toast.warn(
          "To perform a Narrow Search, you must select level of expertise"
        );
        return;
      }
      if (!programmingLanguages.length) {
        toast.warn(
          "To perform a Narrow Search, you must choose atleast one language/tool"
        );
        return;
      }
    }
  ...
  const select = async (topicId, topicCreator) => {
    try {
      const res = await axios.post(
        `${baseUrl}/auth/select-topic`,
        { userId, topicId, topicCreator },
        {
          headers: {
            authorization: cookies.access_token,
          },
