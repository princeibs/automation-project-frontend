...
const StaffUpdateProfile = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        setUpdating(true)
        try {
            const res = await axios.put(`${baseUrl}/staff/update-profile`,
            {
                email: String(email).toLowerCase(), title, firstName, lastName, otherNames, qualifications, publishedDocuments: publishedArticles
            },{
                headers: {
    ...
    const getUserDetails = async () => {
        try {
          ...
          setCurrentDetails(res.data)
            ...
            setOtherNames(res.data.otherNames)
            setQualifications(res.data.qualifications)
            setPublishedArticles(res.data.publishedDocuments)
        } catch (e) {
          toast.error(e?.message)
          console.log(e)
        } finally {
  ...
  return (
      <div class="flex flex-col items-center px-6 sm:px-2 mx-auto w-[35rem] min-h-[80vh] sm:w-full">
        {!pageLoading ? (<div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md">
              <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                  ...
                  <form class="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                      ...
                      <div>
                          <label for="qualifications" class="block mb-2 text-sm font-medium text-gray-900">Published Articles</label>
                          <input type="text" name="qualifications" id="qualifications" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="" value={publishedArticles} onChange={e => setPublishedArticles(e.target.value)}/>
                      </div>
                      {/* <div>
                      ...
                      <button type="submit" disabled={updating} class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center disabled:bg-primary-300">
                      {updating ? (<svg aria-hidden="true" role="status" class="inline w-6 h-6 mr-3 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
